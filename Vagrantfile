...
def ModifyHelmChart(values_file_path)
  ...
  puts "Checking for Helm Chart values.yaml file."
  ...
  if target_line_index.nil?
    ...
    puts "Helm Chart values.yaml 'use_vagrant_config' directive set to true."
  end
end
...
# end
  ...
  puts "\n\n\nChecking for values.yaml.vfbackup"
  ...
  if values_file_path
    if File.exist?(values_file_path + ".vfbackup")
      puts "#{values_file_path + ".vfbackup"} file exists. Reverting changes."
      ...
      puts "No .vfbackup file found. Skipping revert"
    end
  end
  puts "\n\n\n"
